...
class UserModel extends Model {
    ...
    protected $_auto = array(
        array('password', 'user_password_code', self::MODEL_BOTH, 'function', AUTH_KEY),
     ...
     */
    public function register($username, $password, $email, $mobile, $group_id) {
        $data = array(
            'username' => $username,
            'password' => $password,
            'email' => $email,
        ...
        if ($this->create($data)) {
            ...
            $this->addUserInfo($data,$group_id);
    ...
    public function addUserInfo($data=null,$group_id=0){
...
//        RETURN user_password_code($password, AUTH_KEY);
        $map = array();
        switch ($type) {
            case 1:
        ...
        if (is_array($user) && $user['status']) {
            /* 验证用户密码 */
            if (user_password_code($password, AUTH_KEY) === $user['password']) {
