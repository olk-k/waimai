        ...
        <form class="ids">
            <table>
                ...
                <?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td width="700px"><a href="<?php echo U('/Home/Details/trip?i='.$item['pid']);?>"><?php echo ($item["title"]); ?></a></td>
                            <td><?php echo ($item["price"]); ?></td>
                            <td><a href="<?php echo U('/Home/Details/guide?i='.$item['buy_mid']);?>"><?php echo ($item["buy_name"]); ?></a></td>
                            <td><a href="<?php echo U('/Home/Details/guide?i='.$item['sell_mid']);?>"><?php echo ($item["sell_name"]); ?></a></td>
                            <td><?php echo (time_format($item["create_time"])); ?></td>
                            <td><?php echo ($item["ptype_text"]); ?></td>
                            <td><?php echo ($item["status_text"]); ?></td>
                            <td><?php echo ($item["pay_status_text"]); ?></td>
                            <td>
                                <a title="编辑" href="<?php echo U('changeStatus?i='.$item['order_id'].'&s=1');?>" class="ajax-get">结算</a>
                            </td>
