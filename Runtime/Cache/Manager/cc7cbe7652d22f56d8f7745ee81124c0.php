...
<body>
    ...
    <form action="<?php echo U();?>" method="post" class="form-horizontal">
        ...
        <div class="form-item">
            <label class="item-label">用户分组<span class="check-tips">（用户分组用于角色权限分配）</span></label>
            <div class="controls">
                <select name='group_id'>
                    <?php if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$item): $mod = ($i % 2 );++$i;?><!--<option value='<?php echo ($item["id"]); ?>'><?php echo ($item["title"]); ?></option>-->
                        
                        <?php if(($item["id"]) == $info["group_id"]): ?><option value="<?php echo ($item["id"]); ?>" selected ><?php echo ($item["title"]); ?></option>
                        <?php else: ?>
